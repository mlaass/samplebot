...
class RangeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/api/models/"):
            body = json.dumps({"siblings": [{"rfilename": "unet/w.safetensors", "size": len(BLOB)}, {"rfilename": "unet/w.bin", "size": 1},
...
def test_fetch_parallel_ranges_with_retry_and_skips_duplicate_bin(hub, tmp_path):
    ...
    assert not (dest / "unet/w.bin").exists() and (dest / "config.json").exists()
